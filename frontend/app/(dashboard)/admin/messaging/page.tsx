...
interface Message {
  ...
  body: string; category: string; priority: string; createdAt: string;
}
...
export default function AdminMessagingPage() {
  ...
  useEffect(() => {
    fetch('/api/v1/admin/parent-messages').then(r => r.ok ? r.json() : []).then(setMessages);
    fetch('/api/v1/admin/users?role=student').then(r => r.ok ? r.json() : []).then(setStudents);
  }, []);
  ...
  const send = async () => {
    ...
    const res = await fetch('/api/v1/admin/parent-messages', {
      method: 'POST',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><MessageCircle className="h-6 w-6 text-teal-600" />Admin Messaging</h1>
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            ...
            {filtered.length === 0 && (
              <div className="flex flex-col items-center gap-2 py-12 text-gray-400">
                <Mail className="h-8 w-8 text-gray-300" /><p className="text-sm">No messages yet</p>
              </div>
            )}
          </div>
        </div>
          ...
          {compose ? (
            ...
            <div className="flex-1 p-6 overflow-y-auto">
              <div className="mb-4 pb-4 border-b border-gray-100">
                ...
                <Badge variant="outline" className="mt-2 capitalize">{selected.category}</Badge>
              </div>
