...
export default function StudentTimetablePage() {
  ...
  const timetable: Record<string, Array<{
    id: string; name: string; time: string; subject: string; subjectCode?: string;
  ...
  const { data: notices } = useResource<{ id: string; title: string; content: string; publishedAt: string }>('notices');
  if (loading || error) return <ResourceState loading={loading} error={error} />;
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><Clock className="h-6 w-6 text-teal-600" />My Weekly Timetable</h1>
        <p className="text-gray-500 text-sm mt-0.5">Your class schedule for the week</p>
      </div>
      <Card>
          ...
          <table className="w-full text-sm border-collapse min-w-[700px]">
            <thead>
              <tr className="bg-gradient-to-r from-teal-600 to-teal-700 text-white">
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase w-20">Period</th>
                {DAYS.map(day => {
                  const count = (timetable[day] || []).length;
                  return (
                    <th key={day} className="px-3 py-3 text-center text-xs font-semibold uppercase">
                      <div>{day}</div>
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><Sun className="h-4 w-4 text-amber-500" />Upcoming Holidays</CardTitle></CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {notices.filter(n => n.title.toLowerCase().includes('holiday') || n.title.toLowerCase().includes('closed')).map(h => (
