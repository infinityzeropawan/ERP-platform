...
def run_coaching_admin_flow(driver):
    log("SECTION", "COACHING ADMIN E2E FLOW")
    if not login(driver, "admin", "coach@allen.com", "allenpass123"):
...
def run_instructor_flow(driver):
    log("SECTION", "INSTRUCTOR E2E FLOW")
    if not login(driver, "teacher", "instructor@allen.com", "instpass123"):
...
def run_coaching_student_flow(driver):
    log("SECTION", "COACHING STUDENT E2E FLOW")
    if not login(driver, "student", "coach_student@allen.com", "studentpass123"):
...
if __name__ == "__main__":
    log("INFO", "Starting Coaching Center E2E Functional Suite")
    ...
    try:
        run_coaching_admin_flow(driver)
        run_instructor_flow(driver)
        run_coaching_student_flow(driver)
        
        log("SECTION", "COMPLETED COACHING E2E SUITE")
    finally:
        driver.quit()
